...
export default class Custom extends Component {
  ...
  onDragEnd = () => {
    ...
    this.props.upCustomDrag(this.state.items); // for state up drag
  };
  ...
  handle(e, field) {
    ...
    if (field === 'activity') { new_items[e.target.name].activity = e.target.value }
  ...
  handleDelete(index) {
    ...
    // DELETE FROM DB
    axios.delete('/api/custom/' + this.state.items[index].id, {
      headers: {
    ...
    .then(res => {
      ...
      // LIFT STATE UP TO CV
      this.props.upCustomCreateDelete(this.state.items);
    })
  ...
  render() {
    ...
    var listItems = this.state.items.map((link, index) => (// ok
      <div key={index} name="sss" className="d-flex p-1 links" onDragOver={() => this.onDragOver(index)}>
        ...
        <div style={{ display: 'block', width: '100%' }}>
          ...
          <Button color="outline-white dragged-button container" id={'custom_toggler' + index}>
            <div class="row align-items-center">
              <div class="col">
                <div class="m-0 text-left button-title row">
                  {this.state.items[index].activity ? this.state.items[index].activity : '-'}
                  {this.state.items[index].city ? <div dangerouslySetInnerHTML={{ __html: ',&nbsp;'}}></div> : ''}
                  {this.state.items[index].city ? this.state.items[index].city : ''}
                </div>
          ...
          <UncontrolledCollapse toggler={"#custom_toggler" + index} className="roll_button">
            <Card>
              <CardBody>

                <div className="row">
                  <div className="col">
                    <label>{languages_dict[this.props.language]["activity"]}</label>
                    <input
                      key={index}
                      className="form-control draggable-standard-input"
                      value={this.state.items[index].activity}
                      name={index}
                      placeholder={languages_dict[this.props.language]["placeholder_custom_activity"]}
                      autocomplete="off"
                      onChange={(e) => this.handle(e, 'activity')}
                    />
                  </div>
    ...
    return (
      <div>
        <h5><span className="custom-icon"></span>{languages_dict[this.props.language]["custom"]}</h5>
        {listItems}
