...
export default class Languages extends Component {
  ...
  onDragEnd = () => {
    ...
    this.props.upLanguagesDrag(this.state.items);
  };
  ...
  handle(e, field) {
    ...
    if (field === 'language') { new_items[e.target.name].language = e.target.value }
  ...
  handleDelete(index) {
    ...
    // DELETE FROM DB
    axios.delete('/api/languages/' + this.state.items[index].id, {
      headers: {
    ...
    .then(res => {
      ...
      // LIFT STATE UP TO CV
      this.props.upLanguagesCreateDelete(this.state.items);
    })
  ...
  render() {
    ...
    var listItems = this.state.items.map((link, index) => (// ok
      <div key={index} name="sss" className="d-flex p-1 links" onDragOver={() => this.onDragOver(index)}>
        ...
        <div style={{ display: 'block', width: '100%' }}>
          ...
          <Button color="outline-white dragged-button container" id={'languages_toggler' + index}>
            <div class="row align-items-center">
              <div class="col">
                <div class="m-0 text-left button-title row">
                  {this.state.items[index].language ? this.state.items[index].language : '-'}
                </div>
          ...
          <UncontrolledCollapse toggler={"#languages_toggler" + index} className="roll_button">
            <Card>
              <CardBody>

                <div className="row">
                  <div className="col">
                    <label>{languages_dict[this.props.language]["language"]}</label>
                    <input
                      key={index}
                      className="form-control draggable-standard-input"
                      value={this.state.items[index].language}
                      name={index}
                      placeholder={languages_dict[this.props.language]["placeholder_language"]}
                      autocomplete="off"
                      onChange={(e) => this.handle(e, 'language')}
                    />
                  </div>
    ...
    return (
      <div>
        <h5><span className="languages-icon"></span>{languages_dict[this.props.language]["languages"]}</h5>
        {listItems}
